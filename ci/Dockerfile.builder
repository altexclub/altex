...
# altex_hash
RUN git clone https://github.com/altexpay/altex_hash
RUN cd altex_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} altex
RUN useradd -u ${USER_ID} -g altex -s /bin/bash -m -d /altex altex
...
RUN mkdir /altex-src && \
  ...
  chown $USER_ID:$GROUP_ID /altex-src && \
