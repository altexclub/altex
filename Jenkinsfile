...
// which are part of the ALTEX repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="altex-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /altex-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "altexpay-altex/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from altexpay-altex/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'altexpay-altex/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from altexpay-altex/develop"
            }
          }

          if (hasCache) {
            sh "cd /altex-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /altex-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /altex-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /altex-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /altex-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /altex-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /altex-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
