...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all ALTEX related functionality
...
/*
    ...
    allow endless transaction that would bloat ALTEX and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all ALTEX specific functionality
