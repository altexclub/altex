...
const char * const BITCOIN_CONF_FILENAME = "altex.conf";
const char * const BITCOIN_PID_FILENAME = "altexd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "altex" is a composite category enabling all ALTEX-related debug output
                if(ptrCategory->count(std::string("altex"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ALTEX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ALTEX
    // Mac: ~/Library/Application Support/ALTEX
    // Unix: ~/.altex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ALTEX";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ALTEX";
#else
    // Unix
    return pathRet / ".altex";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty altex.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override altex.conf
